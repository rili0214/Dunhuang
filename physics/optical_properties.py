...
class OpticalProperties(nn.Module):
    """
    Physically accurate optical properties module for Dunhuang mural pigments.
    """
    ...
    def _initialize_from_dunhuang_pigments(self):
        ...
        References:
            - Fujii et al., 2022 - Spectral measurements of Dunhuang pigments
    ...
    def _spectral_to_rgb_conversion(self, spectral_values):
        ...
        rgb_flat = rgb_flat / (d65_rgb + 1e-8)
    ...
    def _kubelka_munk_mixing(self, absorption_coeffs, scattering_coeffs, concentrations=None):
        ...
        References:
            - Saunderson, 1942 - Calculation of the color of pigmented plastics
            - Kubelka & Munk, 1931 - Ein Beitrag zur Optik der Farbanstriche
        ...
        # Apply concentrations to K/S ratios rather than directly to K and S
        k_over_s_values = []
    ...
    def _compute_mie_scattering(self, wavelengths, particle_size, refractive_index):
        ...
        References:
            ...
            - Wiscombe, 1980 - Improved Mie scattering algorithms
        ...
        # Rayleigh scattering formula (analytical approximation)
        ...
        if torch.any(rayleigh_mask):
            ...
            m_squared = m * m
            scattering_efficiency[rayleigh_mask] = (8/3) * (x**4) * torch.abs((m_squared - 1)/(m_squared + 2))**2 * 5.0  # Amplified by factor of 5
            
            # Absorption efficiency depends on imaginary part
            absorption_efficiency[rayleigh_mask] = 4 * x * torch.imag(m)
            
            # Asymmetry = 0 for Rayleigh scattering (isotropic)
            asymmetry[rayleigh_mask] = 0.0
                
        # Mie regime, use approximation from Wiscombe, 1980
        ...
        if torch.any(geometric_mask):
            ...
            # Scattering efficiency approaches 2 in geometric limit
            ...
            # Asymmetry approaches 1 (forward scattering)
    ...
    def _apply_particle_size_effects_vectorized(self, wavelengths, spectral_reflectance, particle_data):
        """
        Vectorized version of particle size effects using Mie theory.
        
        Args:
            wavelengths: Light wavelengths [spectral_bands]
            spectral_reflectance: Original spectral reflectance [batch_size, spectral_bands]
            particle_data: Dict with particle size information
            
        Returns:
            Modified spectral reflectance [batch_size, spectral_bands]
        """
        ...
        # Convert reflectance to Kubelka-Munk parameters
        # R = 1 + K/S - sqrt((K/S)^2 + 2*K/S)
        # Solve for K/S = (1-R)^2 / (2*R)
        K_over_S = (1 - spectral_reflectance)**2 / (2 * spectral_reflectance.clamp(min=1e-6))
        
        # Scale factor based on particle size relative to reference size (5 microns)
        size_factor = mean_size / 5.0
        
        # Apply size-dependent effect: smaller particles absorb more
        modified_K_over_S = K_over_S * (1.0 / size_factor.clamp(min=0.2))

        modified_reflectance = 1 + modified_K_over_S - torch.sqrt(modified_K_over_S**2 + 2*modified_K_over_S)
        modified_reflectance = modified_reflectance.clamp(0.0, 1.0)
    ...
    def apply_spectral_aging(self, spectral_values, aged_spectral_values):
        ...
        return self._spectral_to_rgb_conversion(aged_spectral_tensor)
    ...
    def _calculate_layered_reflectance(self, layer_properties):
        """
        Calculate effective reflectance of multiple pigment layers using
        Kubelka-Munk multi-layer theory with wavelength-dependent complex IOR
        
        References:
            - Kubelka, 1954 - New contributions to the optics of intensely light-scattering materials
            - Palik, 1998 - Handbook of Optical Constants of Solids
    ...
    def forward(self, pigment_indices, mixing_ratios=None, precomputed_aging=None, 
        ...
        # Handle specific physical effects
    ...
    def simulate_full_rendering(self, 
        ...
        References:
            ...
            - Thoury et al., 2016 - Synchrotron-based photoluminescence approach for pigment degradation
        ...
        if age_years > 0:
            if thermodynamic_validator is not None:
                ...
                for b in range(batch_size):
                    ...
                    age_factor = age_years / 100.0  # Normalize to 100 years
        ...
        if viewing_angle != 0.0:
            ...
            # Smith masking-shadowing term (Heitz, 2014)
